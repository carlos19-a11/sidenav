        ...
        >
          ...
          <i
            *ngIf="item.items && collapsed"
            ...
            [ngClass]="
              !item.expanded ? 'fal fa-angle-ringht' : 'fal fa-angle-down'
            "
          ></i>
        </a>
        <a
  ...
  handleClick(item: any): void {
    if (!this.multiple) {
      if (this.data.items && this.data.items.length > 0) {
        for (let modeLiItem of this.data.items) {
          if (item! == modeLiItem && modeLiItem.expanded) {
            modeLiItem.expanded = false;
          }
        }
      }
    }
